#!/usr/bin/env python3
"""Leakage-safe, full-volume evaluation for patient-level BraTS splits.
...
def main():
    ...
    for index, (case_id, case_path) in enumerate(zip(evaluation_ids, case_paths), start=1):
        ...
        rows.append(row)
        print(f"[{index}/{len(case_paths)}] {case_id}: Dice WT={metrics['dice_wt']:.6f}")

    with args.output_csv.resolve().open("x", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(handle, fieldnames=CSV_COLUMNS)
        writer.writeheader()
        writer.writerows(rows)
    summary = {
        "split": args.split, "fold": args.fold, "modalities": list(args.modalities),
    ...
    with output_json.open("x", encoding="utf-8") as handle:
        ...
        handle.write("\n")
    print(f"Saved {args.output_csv.resolve()} and {output_json}")
