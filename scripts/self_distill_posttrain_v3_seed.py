#!/usr/bin/env python3
"""Numerically stable self-distillation with anchored full-modality behavior."""
...
V3_TEACHER_SHA256 = "c24cb1ce804abfba205fc660bd762b520b949180d2e8d4e111bc2ee576fdd3c4"
...
def ensure_gpu_idle() -> None:
    ...
    for line in completed.stdout.splitlines():
        ...
        if len(fields) != 2:
            continue
        try:
            pid, used_memory_mb = int(fields[0]), int(fields[1])
        except ValueError:
            continue
        if pid != os.getpid() and used_memory_mb > 1024:
...
def payload(epoch, student, optimizer, args, split_sha256, teacher_baseline,
    ...
    return {
        ...
        "source_checkpoint_sha256": args.teacher_sha256,
...
def main() -> None:
    ...
    if checkpoint_dir.exists() or result_dir.exists():
        raise FileExistsError(
            f"Refusing to reuse seed outputs: {checkpoint_dir} / {result_dir}"
        )

    ensure_gpu_idle()
    device = torch.device(args.device)
    if device.type != "cuda" or not torch.cuda.is_available():
        raise RuntimeError("V3 post-training requires a free CUDA device")
    ...
    if checkpoint_path != expected_source or not checkpoint_path.is_file():
        raise ValueError(f"V3 must initialize from the original teacher: {expected_source}")
    if not split_json.is_file():
        raise FileNotFoundError(split_json)

    teacher_sha256 = file_sha256(checkpoint_path)
    if teacher_sha256 != V3_TEACHER_SHA256:
        raise RuntimeError(
            f"Teacher SHA mismatch: expected v3 teacher {V3_TEACHER_SHA256}, "
            f"found {teacher_sha256}"
        )
    resolved_config = {
        "seed": args.seed,
        ...
        "w_feat": args.w_feat,
        "temperature": args.temperature,
        "kd_loss": "KL(log_softmax(student/T), softmax(teacher/T)) * T^2",
        "feature_loss": "mean SmoothL1(beta=1.0) over fused_bottleneck and aggregated_skips",
        "mask_probabilities": {
            "full_anchor": 0.45,
            "drop_t1ce": 0.35,
            "random_subset": 0.20,
        },
        "random_subset_keep_count": "uniform_integer_1_to_3",
        "selection": "max drop_t1ce mean(TC,ET) subject to all_mean >= 0.97*teacher_all_baseline",
        "autocast_dtype": "bfloat16",
        "grad_scaler": False,
        "max_grad_norm": 1.0,
        "batchnorm_running_stats_frozen": True,
        "batchnorm_affine_frozen": True,
        "dataloader_worker_seeding": "torch.initial_seed derived from seeded generator",
    }
    print("RESOLVED_CONFIG " + json.dumps(resolved_config, sort_keys=True), flush=True)
    print(f"TEACHER checkpoint={checkpoint_path} sha256={teacher_sha256}", flush=True)
    args.teacher_sha256 = teacher_sha256
    ...
    with epoch_path.open("x", newline="", encoding="utf-8") as epoch_handle, \
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            for step, (images, targets) in enumerate(train_loader, start=1):
                ...
                availability, category = v2.sample_v2_mask(mask_rng, device)
            ...
            if epoch % args.val_every == 0 or epoch == args.epochs:
                ...
                if constraint_met and drop_selection > best_score:
                    ...
                    save_checkpoint_atomic(
                        checkpoint_dir / f"best_self_distill_v3_seed{args.seed}.pth",
                        payload(
            ...
            epoch_handle.flush()
            save_checkpoint_atomic(
                checkpoint_dir / f"last_self_distill_v3_seed{args.seed}.pth",
                payload(
                    epoch, student, optimizer, args, split_sha256,
                    teacher_baseline, constraint_threshold, all_scores,
                    drop_scores, best_epoch, best_score,
