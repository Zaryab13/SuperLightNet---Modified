#!/usr/bin/env python3
"""Numerically stable, drop-T1ce-specialized self-distillation v4."""
...
CHECKPOINT_DIR = PROJECT_ROOT / "checkpoints" / "self_distill_v4"
RESULT_DIR = PROJECT_ROOT / "results" / "03_self_kd" / "self_distill_v4"
...
def main() -> None:
    ...
    if CHECKPOINT_DIR.exists() or RESULT_DIR.exists():
        raise FileExistsError(
            f"Refusing to reuse v4 outputs: {CHECKPOINT_DIR} / {RESULT_DIR}"
        )

    device = torch.device(args.device)
    if device.type != "cuda" or not torch.cuda.is_available():
        raise RuntimeError("V4 post-training requires a free CUDA device")
    ...
    if checkpoint_path != expected_source or not checkpoint_path.is_file():
        raise ValueError(f"V4 must initialize from the original teacher: {expected_source}")
    ...
    epoch_path = RESULT_DIR / "self_distill_v4_training_log.csv"
    batch_path = RESULT_DIR / "self_distill_v4_training_batch_log.csv"
    tradeoff_path = RESULT_DIR / "self_distill_v4_validation_tradeoff.csv"
    with epoch_path.open("x", newline="", encoding="utf-8") as epoch_handle, \
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            if epoch % args.val_every == 0 or epoch == args.epochs:
                ...
                if constraint_met and drop_selection > best_score:
                    ...
                    save_checkpoint_atomic(
                        CHECKPOINT_DIR / "best_self_distill_v4.pth",
                        payload(
            ...
            epoch_handle.flush()
            save_checkpoint_atomic(
                CHECKPOINT_DIR / "last_self_distill_v4.pth",
                payload(
                    epoch, student, optimizer, args, split_sha256,
                    teacher_baseline, constraint_threshold, all_scores,
                    drop_scores, best_epoch, best_score,
