...
from scripts.evaluate_swin_kd_clean_B_macaf import (  # noqa: E402
    MODEL_MODALITIES,
    case_metrics,
    load_case,
...
def validate_subset(model: MultiEncoderRMDUNet, case_ids: Sequence[str], dataset_root: Path,
    ...
    avail_mask = modality_availability(modalities, device)
    for index, case_id in enumerate(case_ids, start=1):
        volume, ground_truth, spacing = load_case(
            dataset_root / case_id, modalities,
        )
        prediction = sliding_window_predict(
            model, volume, device, roi_size, overlap, avail_mask=avail_mask,
        )
