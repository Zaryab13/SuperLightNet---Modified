#!/usr/bin/env python3
"""Train a SuperLightNet ablation on leakage-safe patient-level splits."""
...
def validate_output_locations(output_dir: Path, resume: Path | None):
    ...
    protected = (checkpoints_root / "leakage_safe").resolve()
    output_dir = output_dir.resolve()
    if output_dir.parent != checkpoints_root or output_dir == protected:
        raise ValueError(f"--output_dir must be a direct ablation subdirectory of {checkpoints_root}")
    output_dir.mkdir(parents=True, exist_ok=True)
    best_path = output_dir / "best_patient_split.pth"
    last_path = output_dir / "last_patient_split.pth"
    results_dir = PROJECT_ROOT / "results" / "01_base_model" / output_dir.name
    log_path = results_dir / f"{output_dir.name}_training_log.csv"
    batch_log_path = results_dir / f"{output_dir.name}_training_batch_log.csv"
...
def load_resume_state(path: Path, model, optimizer, scaler, manifest_sha256: str,
    ...
    if last_logged_epoch(log_path) != completed_epoch:
        raise ValueError("The ablation training log and last checkpoint disagree on the completed epoch")
    ...
    return (
        ...
        int(checkpoint.get("validated_epochs_without_improvement", 0)),
        bool(checkpoint.get("minimum_dice_check_completed", False)),
    )
...
def main():
    ...
    parser.add_argument("--output_dir", type=Path, default=Path("checkpoints/ablation_no_rmd"))
    ...
    parser.add_argument("--rmd_enable", type=parse_bool, choices=(True, False), default=True)
    parser.add_argument("--val_every", type=int, default=1)
    args = parser.parse_args()

    if (args.epochs < 1 or args.batch_size < 1 or args.lr <= 0 or args.val_every < 1 or
            args.num_workers < 0 or args.val_workers < 0):
        parser.error("epochs, batch_size, learning rate, and val_every must be positive; workers cannot be negative")
    ...
    print(f"MODEL CONFIG: rmd_enable={model.rmd_enable}")
    ...
    validated_epochs_without_improvement = 0
    minimum_dice_check_completed = False
    start_epoch = 1
    if resume_path is not None:
        (start_epoch, best_validation_dice, validated_epochs_without_improvement,
         minimum_dice_check_completed) = load_resume_state(
        ...
        for epoch in range(start_epoch, args.epochs + 1):
            ...
            if should_validate:
                ...
                if selection_dice > best_validation_dice:
                    ...
                    validated_epochs_without_improvement += 1
                if epoch >= MINIMUM_DICE_EPOCH and not minimum_dice_check_completed:
                    minimum_dice_check_completed = True
                    if selection_dice < MINIMUM_DICE:
                        stop_reason = (
                            f"EARLY STOP: epoch {epoch} is the first validation on/after epoch "
                            f"{MINIMUM_DICE_EPOCH}, and mean region Dice {selection_dice:.6f} < {MINIMUM_DICE:.2f}"
                        )
                if (stop_reason is None and
                        validated_epochs_without_improvement >= EARLY_STOPPING_PATIENCE):
                    stop_reason = (
                        f"EARLY STOP: no new best validation Dice for "
                        f"{EARLY_STOPPING_PATIENCE} validated epochs"
                    )
            ...
            checkpoint = {
                ...
                "scaler": scaler.state_dict(), "best_val_dice": best_validation_dice,
                ...
                "rmd_enable": args.rmd_enable, "val_every": args.val_every,
                "validated_epochs_without_improvement": validated_epochs_without_improvement,
                "minimum_dice_check_completed": minimum_dice_check_completed,
            }
            ...
            if should_validate:
                ...
                print(
                    f"Epoch {epoch}: train_loss={train_loss:.6f} val_loss= "
                    f"WT= TC= ET= time={elapsed:.1f}s"
                )
