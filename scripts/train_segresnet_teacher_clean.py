#!/usr/bin/env python3
"""Train a leakage-clean SegResNet teacher from random initialization on PC3."""
...
CHECKPOINT_DIR = PROJECT_ROOT / "checkpoints" / "segresnet_teacher_clean"
RESULT_DIR = PROJECT_ROOT / "results" / "segresnet_teacher_clean"
BEST_PATH = CHECKPOINT_DIR / "best.pth"
LAST_PATH = CHECKPOINT_DIR / "last.pth"
LOG_PATH = RESULT_DIR / "training_log.csv"

SUPPORTED_MONAI_VERSIONS = ("1.4.0", "1.5.0", "1.5.2")
...
OUR_CHOICE_LINES = (
    "[MODEL CHANGE ONLY: SegResNet replaces SwinUNETR; the clean teacher recipe is unchanged.]",
    "[MODEL: SegResNetVAE is intentionally omitted.]",
...
def main() -> None:
    ...
    if monai_version not in SUPPORTED_MONAI_VERSIONS:
        raise RuntimeError(
            f"Unsupported MONAI {monai_version}; expected one of {SUPPORTED_MONAI_VERSIONS}. "
            "Use the pinned PC3 requirements and verify the exact parameter-count assertion."
        )
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is required for clean SegResNet teacher training")
    ...
    print("Only the model differs from the Swin teacher recipe.", flush=True)
    print(f"MONAI version: {monai_version}", flush=True)
    ...
    if param_count != EXPECTED_PARAM_COUNT:
        raise RuntimeError(
            f"Unexpected SegResNet parameter count: {param_count}; expected {EXPECTED_PARAM_COUNT}"
        )
    ...
    print(
        ...
        "No existing files modified. Test set never accessed.",
        flush=True,
    )
